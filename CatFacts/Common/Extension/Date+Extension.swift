...
extension Date {
    ...
    func toDateString(format: String = "EEE, MMMM d, yyyy") -> String? {
...
extension Date {
    ...
    func mins(from date: Date) -> Int {
        return Calendar.current.dateComponents([.minute], from: date, to: self).minute ?? 0
    }
    
    func secs(from date: Date) -> Int {
    ...
    func offset(from date: Date) -> String {
        ...
        if mins(from: date) > 0 { return "\(mins(from: date))m" }
        if secs(from: date) > 0 { return "\(secs(from: date))s" }
        return ""
    }
}
